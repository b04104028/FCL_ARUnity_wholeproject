    ...
    public class PrefabImagePairManager : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        void AssignPrefab(ARTrackedImage trackedImage)
        {
            if (m_PrefabsDictionary.TryGetValue(trackedImage.referenceImage.guid, out var prefab))
            {
                ...
                m_Instantiated[trackedImage.referenceImage.guid] = Instantiate(prefab, model_loc, transform.rotation * Quaternion.Euler(270f, 180f, 0f));//(0f, 180f, 180f));//Quaternion.identity);
