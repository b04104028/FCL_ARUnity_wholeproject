...
public class QCColorManager : MonoBehaviour
{
    ...
    public void Start()
    {
        string path = Application.persistentDataPath + "/PersistantFilePath/" + "Zurich_QC_total.json";//"ExampleJSON.json";
    ...
    public void AssignColor()
    {
        ...
        {
            case <= 0.2f:
                rend.material.color = Color.Lerp(transparentWhite, transparentBlue, (float)(lerp / 0.2f));
                break;
            case <= 1.0f:
                rend.material.color = Color.Lerp(transparentBlue, Color.black, (float)((lerp - 0.2f) / 0.8f));
                break;
            case > 1.0f:
