...
public class EColorbar : MonoBehaviour
{
    public List<GameObject> colorbarlist = new List<GameObject>();
    [SerializeField] public EColorManager _ecolormanager;
    ...
    {
        SetColorbar();
        double maxloadrounded = RoundToNearest(_ecolormanager.max_load);
    ...
    public void SetColorbar()
    {
        ...
        foreach (GameObject colorbar in colorbarlist)
        {
            ...
            if (lerp <= 1)
            {
                colorbar.GetComponent<Image>().color = Color.Lerp(Color.black, Color.green, lerp);            
            }
            else if (lerp > 1)
            {
                lerp = 0.2f * (i - 5);
                colorbar.GetComponent<Image>().color = Color.Lerp(Color.green, Color.white, lerp);
            }
